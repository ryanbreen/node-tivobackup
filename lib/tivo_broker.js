...
exports.processShowDetailRequest = function(req, recording_metadata, metadata, cb) {
...
exports.processListingRequest = function(req, recording_metadata, cb) {
			...
			if (result || result.TiVoContainer || result.TiVoContainer.Item) {
				...
				result.TiVoContainer.Item.forEach(function(item) {
					...
					if (item.Details && item.Details.TotalItems && item.Links && item.Links.Content && item.Links.Content.Url) {
						...
						recording_metadata.pending_count += 1;
						exports.processListingRequest(new_req, recording_metadata, cb);
					} else {
						if (item.Details && item.Links && item.Links.Content && item.Links.Content.Url &&
							...
							recording_metadata.pending_count += 1;
							exports.processShowDetailRequest(new_req, recording_metadata, metadata, cb);
						}
					}
				});
			} else {
...
exports.findRecordings = function(tivo, cb) {
	...
	exports.processListingRequest(req, recording_metadata, cb);
};
