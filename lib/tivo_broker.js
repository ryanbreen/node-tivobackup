		...
		parseString(xml, {trim: true, mergeAttrs: true, explicitArray: false}, function (err, result) {
			...
			if (result || result.TiVoContainer || result.TiVoContainer.Item) {
				...
				if (result.TiVoContainer.Details && result.TiVoContainer.ItemCount &&
					...
					var start_from = parseInt(result.TiVoContainer.ItemCount) + parseInt(result.TiVoContainer.ItemStart);

					var new_req = _.clone(req);
					new_req.url = 'https://' + tivo.ip + '/TiVoConnect?Command=QueryContainer&Details=All&Container=%2FNowPlaying&ItemStart=' + start_from;
					recordings.pending_count += 1;
				...
				result.TiVoContainer.Item.forEach(function(item) {
					...
					if (item.Details && item.Details.TotalItems && item.Links && item.Links.Content && item.Links.Content.Url) {
						...
						recordings.pending_count += 1;
						exports.processListingRequest(tivo, new_req, recordings, cb);
					} else if (item.Details && item.Links && item.Links.Content && item.Links.Content.Url &&
...
exports.findRecordings = function(tivo, cb) {
	...
	var req = {
		...
		'url': 'https://' + tivo.ip + '/TiVoConnect?Command=QueryContainer&Details=All&Container=%2FNowPlaying&ItemCount=25',
		'auth': {
			'user': 'tivo',
			'pass': tivo.mak,
