...
module.exports = function(store) {
	...
	return new function() {
		...
		this.recordingExists = function(recording) {
			...
			return fs.existsSync(recording.recording_path);
		};
		...
		var downloadRecording = function(tivo, recording, cb) {
			// Start the download.
			downloader.downloadFile(tivo, recording, function(err) {
				if (err) return cb(err);

				logger.info('Completed download of %s from tivo %s', recording.title, tivo.ip);
				cb();
			});
		};
	}();
